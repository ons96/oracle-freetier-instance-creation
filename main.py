...
OCI_IMAGE_ID = (
    os.getenv("OCI_IMAGE_ID", None).strip() if os.getenv("OCI_IMAGE_ID") else None
)
OCI_COMPUTE_SHAPE = os.getenv("OCI_COMPUTE_SHAPE", ARM_SHAPE).strip()
SECOND_MICRO_INSTANCE = (
    os.getenv("SECOND_MICRO_INSTANCE", "False").strip().lower() == "true"
)
OCI_SUBNET_ID = (
    os.getenv("OCI_SUBNET_ID", None).strip() if os.getenv("OCI_SUBNET_ID") else None
)
...
def validate_always_free_compliance():
    ...
    logging.info(
        "✅ Region: %s",
        oci_region
        if oci_region in ALWAYS_FREE_REGIONS
        else "⚠️ NOT ALWAYS-FREE ELIGIBLE",
    )
    logging.info(
        "✅ Boot Volume: %sGB (within %sGB Always-Free limit)",
        max(50, boot_volume_int),
        ALWAYS_FREE_MAX_STORAGE_GB,
    )
...
if OCI_CONFIG:
    ...
    oci_config_path = (
        OCI_CONFIG
        if os.path.isabs(OCI_CONFIG)
        else os.path.join(os.getcwd(), OCI_CONFIG)
    )
else:
...
def check_instance_state_and_write(
    compartment_id, shape, states=("RUNNING", "PROVISIONING"), tries=3
):
...
def handle_errors(command, data, log):
    ...
    if "code" in data:
        if (
            data["code"]
            in ("TooManyRequests", "Out of host capacity.", "InternalError")
...
def generate_ssh_key_pair(
    public_key_file: Union[str, Path], private_key_file: Union[str, Path]
):
...
def launch_instance() -> bool:
    ...
    if not oci_subnet_id:
        subnets = execute_oci_command(
            network_client, "list_subnets", compartment_id=oci_tenancy
        )
    ...
    logging.info(
        "✅ Boot volume size validated: %sGB (within 100GB safe limit)",
        boot_volume_size,
    )
    ...
    instance_exist_flag = check_instance_state_and_write(
        oci_tenancy, OCI_COMPUTE_SHAPE, tries=1
    )
    ...
    while not instance_exist_flag:
        if MAX_RUNTIME_SECS and (time.monotonic() - start_time) >= MAX_RUNTIME_SECS:
            ...
            write_into_file(
                os.path.join(os.getcwd(), "MAX_RUNTIME_REACHED"), msg + "\n"
            )
            return False
        ...
        logging_step5.info(
            "🎯 Attempting instance creation in AD: %s (Attempt %d)",
            current_ad,
        ...
        try:
            ...
            if launch_instance_response.status == 200:
                ...
                if instance_exist_flag:
                    logging_step5.info(
                        "🎉 Instance successfully created in AD: %s", current_ad
                    )
                    break
            ...
            if srv_err.code == "LimitExceeded":
                logging_step5.info(
                    "Encountered LimitExceeded Error checking if instance is created"
                ...
                srv_err.code in ("OutOfCapacity", "Out of host capacity")
                or "capacity" in srv_err.message.lower()
            ):
                logging_step5.warning(
                    "🚨 Capacity error in AD %s: %s. Trying next AD...",
                    current_ad,
                    srv_err.message,
                )
                ...
                if current_ad_index >= len(oci_ad_name):
                    logging_step5.info(
                        "⏳ All ADs tried once. Starting new cycle of AD attempts..."
                    )
                    current_ad_index = 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        if created:
            send_discord_message(
                "🎉 Success! OCI Instance has been created. Time to celebrate!"
            )
        else:
            send_discord_message(
                "⏱️ No capacity yet. Max runtime reached; will try again later."
            )
